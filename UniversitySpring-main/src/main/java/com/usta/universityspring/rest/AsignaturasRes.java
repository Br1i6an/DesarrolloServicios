...
public class AsignaturasRes {
    ...
    @GetMapping (value = "/totalAsignaturas")
    private ResponseEntity<String> listTotalAsignaturasPorID (@PathVariable ("id")Long id){

        asignaturaServicio.eliminarAsignatura(id);
        return ResponseEntity.ok("Asignatura eliminada con éxito"+ String.valueOf(asignaturaServicio.));
    }
}
