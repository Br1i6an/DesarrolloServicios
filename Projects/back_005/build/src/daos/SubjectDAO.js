...
class SubjectDAO {
    static getSubjects(sqlConsult, parameters, res) {
    ...
    static createSubjects(sqlConfirm, sqlCreate, parameters, res) {
        return __awaiter(this, void 0, void 0, function* () {
            yield connectionDb_1.default
                ...
                if (responses.id_partido != 0) {
                    res.status(200).json({ responses: 'Subject Created Succesfully ',
                        nuevoCodigo: responses.program_id });
                }
                else {
                    res.status(402).json({ responses: 'Error creating Subject, \
